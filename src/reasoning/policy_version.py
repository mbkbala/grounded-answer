...
class PolicyVersion:
    ...
    def apply_to_text(
        ...
        """Apply the amendment text relevant to one clause."""

        if resolution.get("status") != "amended_rule":
            return original_text

        amended_text = original_text
        clause = normalize_clause_reference(clause_id)

        replacements = {
            "§6.4.1": [("$120", "$175")],
            "§4.3.2": [("10 calendar days", "14 calendar days")],
            "§9.1.4": [("30 calendar days", "14 calendar days")],
            "§10.5.2": [("20 per cent", "15 per cent")],
        }

        for old, new in replacements.get(clause, []):
            amended_text = amended_text.replace(old, new)
